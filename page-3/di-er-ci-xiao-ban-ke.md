...
> 3. 第三，对于未定义行为引发的任何后果，编译器概不负责。因为未定义行为本身就是不受法律（即标准）保护的。编译器的职责，只是为符合标准的“已定义行为”生成高效的代码（汇编及可执行文件）[8](https://www.cnblogs.com/zpcdbky/p/14853401.html#annotation8)。
