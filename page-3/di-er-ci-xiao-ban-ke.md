...
<pre class="language-cppp"><code class="lang-cppp">#include&#x3C;stdio.h>
int main()
{
    int i=0;
    int a[] = {10,20,30};
    ...
    int j=4;
    j=j++;
<strong>    printf("i=%d,j=%d,r=%d",i,j,r);
</strong>    return 0;
}
</code></pre>

编译器精神开始分裂，变成三个编译器分别开始工作，两位寄存器出场

#### GCC编译器

`i=3,j=4,r=140`

编译器要求寄存器2 +10，寄存器1++；寄存器2 +2\*20，寄存器1++；寄存器2 +3\*20，寄存器1++；

而在`j=j++`时，寄存器1、2分别保存j和j+1的值，到最后却让寄存器1输出

#### ICC编译器

`i=3,j=5,r=60`

编译器开始读代码，在读到 `int r = 1 * a[i++] + 2 * a[i++] + 3 * a[i++];`时要求寄存器1先+10，再+20，再+30；再要求寄存器2  +1 +1 +1；最终输出r=60；寄存器表示非常不理解

#### Clang编译器

编译器虽然是错误的，但是给出了未定义行为的警告

两个寄存器吵了起来，编译器和程序员吵了起来，互相推卸责任，此时旁白出场

### 第三幕 旁白出场解释原因

### 第四幕 包饺子

旁白：”编译器，你要学会给出处理未定义行为的报错；寄存器要有自己的识别能力；程序员要写出合理的代码；我们一起让程序运行起来！来，给同学们拜个早年！我们一起包饺砸！“
...
```
movsxd    rcx, ecx 
其中sxd:sign-extend double word,将双字符号扩展
将源操作数的符号位进行扩展，从而保留原来的符号，然后将结果存储到目的操作数中
将32位的ecx寄存器的值符号扩展到64位的rcx
```

```
imul rcx, rcx, 4
rcx 寄存器既是源操作数也是目标操作数，表示进行自身的乘以 4 操作。
有符号整数的乘法运算
格式为 imul dest, src1, src2
dest=src1*src2
src1 和 src2
```
...
```
cdqe
将存储在eax寄存器中的32位整数符号扩展为64位，并存储在rax寄存器中。
如果eax中的值为0xFFFFFFFF，即-1的补码表示
那么cdqe指令会将rax的值设置为0xFFFFFFFFFFFFFFFF，即-1的补码表示
因为符号扩展将高32位填充为1。
```
...
[https://blog.csdn.net/Edidaughter/article/details/122334074](https://blog.csdn.net/Edidaughter/article/details/122334074)

在函数调用期间，函数需要使用一些寄存器来存储临时变量、参数、返回值等信息，而这些寄存器可能同时被调用者和调用者使用，导致使用冲突。调用者保存和被调用者保存的设计，就是为了解决这种冲突。

**调用者保存**可以让编译器在函数调用过程中使用所有寄存器，而不用考虑哪些寄存器需要在函数调用期间被保存。但是，这也会增加代码的开销和复杂度，因为调用者需要保存和恢复所有使用的寄存器。

**被调用者保存**可以让编译器选择哪些寄存器可以被修改，因为它们需要在函数调用期间被保存。这会限制编译器使用的寄存器数量，因此需要更加谨慎地使用寄存器。

因此，调用者保存和被调用者保存的设计是为了平衡使用寄存器的开销和复杂度。调用者保存适用于函数调用次数较少、使用寄存器较少的场景；而被调用者保存适用于函数调用次数较多、使用寄存器较多的场景。
